...
/** Taille réelle en octets depuis la longueur base64 (approximation) */
...
/** Renvoie une data-URL complète quel que soit le format reçu */
...
/** Raw base64 sans préfixe, pour écriture disque */
...
export function FileTypeIcon({ mime, size = 20, className = "" }: { mime: string; size?: number; className?: string }) {
  const cat = mimeToCategory(mime);
  const s = size;
  const shared = `w-[${s}px] h-[${s}px] shrink-0 ${className}`;

  switch (cat) {
    case "image":
      return (
        <svg className={shared} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5">
...
/* ─── Vignette pour la liste de fichiers ──────────────────────────── */
...
/* ─── Lightbox / modal de prévisualisation ────────────────────────── */
...
function PreviewModal({ attachment: a, onClose, onDownload }: PreviewModalProps) {
  ...
  const renderContent = () => {
    switch (cat) {
      ...
      case "pdf":
        return (
          <iframe
            src={dataUrl}
            title={a.filename}
            className="w-full h-[60vh] rounded-xl border border-edge"
          />
        );
      case "text":
        return (
      ...
      case "audio":
        return (
          <audio controls src={dataUrl} className="w-full rounded-lg" />
        );
      case "video":
        return (
          <video controls src={dataUrl} className="max-w-full max-h-[55vh] rounded-xl" />
        );
      default:
        return (
...
/** Affichage texte — décode le base64 et affiche en bloc scrollable */
function TextPreview({ a }: { a: Attachment }) {
  const [text, setText] = useState<string | null>(null);

  useEffect(() => {
    try {
      const raw = rawBase64(a);
      setText(atob(raw));
    } catch {
...
interface AttachmentListProps {
  ...
  /** Si true, affiche les boutons Retirer (mode édition) */
  editable?: boolean;
...
export function AttachmentList({ attachments, editable, onRemove, onAdd, addError }: AttachmentListProps) {
  ...
  const handleDownload = async (a: Attachment) => {
    try {
      ...
      setTimeout(() => setDownloadStatus((s) => { const n = { ...s }; delete n[a.id]; return n; }), 2000);
    } catch {
      setDownloadStatus((s) => ({ ...s, [a.id]: "err" }));
      setTimeout(() => setDownloadStatus((s) => { const n = { ...s }; delete n[a.id]; return n; }), 3000);
    }
  };

  return (
    <>
      {/* Label + bouton ajouter (mode édition) */}
      {editable && (
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs uppercase tracking-wider text-muted">
            Pièces jointes <span className="normal-case text-muted/60">(max 3 Mo)</span>
          </label>
          <button
            type="button"
            onClick={onAdd}
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted leading-tight">{formatBytes(base64Bytes(a.data_base64))}</p>
                </div>
                ...
                {/* Actions toujours visibles en mode édition pour Retirer */}
                {editable && onRemove && (
                  <button
                    type="button"
                    onClick={() => onRemove(a.id)}
